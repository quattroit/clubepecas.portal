...
/**
 * Detalhe de uma cotação enviada pelo comprador profissional — somente leitura.
 */
function QuotationDetailView() {
  const params = useParams<{ id: string }>();
  const id = parseRouteId(params.id);

  const detailQuery = useMyQuotation(id ?? 0, Boolean(id));
  ...
  if (!id) {
    return (
      <NotFound
        ...
        homeHref={ROUTES.PROFESSIONAL_BUYER_QUOTATIONS_HISTORY}
      />
    );
  }
  ...
  if (detailQuery.isError) {
    if (detailQuery.error instanceof NotFoundError) {
      return (
        <NotFound
          title="Cotação não encontrada"
          description="Esta cotação não existe ou não pertence à sua conta."
          homeHref={ROUTES.PROFESSIONAL_BUYER_QUOTATIONS_HISTORY}
        />
      );
    }
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-small text-muted-foreground">
            Enviada em {formatDate(data.submittedAtUtc)} às{" "}
      ...
      <Card>
        ...
        <CardContent>
          <dl className="grid grid-cols-2 gap-4 sm:grid-cols-3">
            <DetailField label="Loja" value={data.storeName} />
            <DetailField
              label="WhatsApp"
          ...
          {data.sellerWhatsApp ? (
            <a
              href={`https://wa.me/${data.sellerWhatsApp.replace(/\D/g, "")}`}
              target="_blank"
